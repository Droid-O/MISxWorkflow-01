...
- **Monthly Plan — a full month of posts:** Pick a venue and month (and list any known events). It proposes the whole month's content as a calendar table — date, post type, concept, and the final EN/AR captions and hashtags for each — mapped to the fields in the venue's presentation template. Export as CSV to drop straight into the deck, or **Copy captions** to paste the whole month, in order, into a Google Doc.
- **Rename files — month-end export naming:** Drop the month's exported images. It auto-detects each file's size (1080×1080 / 1350 / 1920), groups the sizes of each post by their shared file name, and renames everything to the convention (`Venue_Post_04 (1080x1350)`). Preview the mapping, then download them all renamed.

Stage 1 and Stage 2 can also append a rough **SVG layout sketch** — a throwaway comp showing composition, EN/AR headline placement, and the venue's colour world — to hand to the designer alongside the brief.
